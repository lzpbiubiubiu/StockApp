...
#include <QVariantMap>
#include "Base/Database/Orm/QdOrmDml.h"
#include "Base/Database/Orm/QdOrmFactory.h"
#include "Base/Database/Orm/QdOrmCondition.h"
...
namespace Orm
{
    ...
    public:
        static inline QSqlError fetchByQuery(T& t, QSqlDatabase* database, const QdOrmWhere& condition)
        {
            using OriginalType = typename std::remove_reference<typename std::remove_pointer<typename T::value_type::value_type>::type>::type;
            ...
            while (query.next())
            {
                typename T::value_type row = T::value_type::create();
                foreach(const OrmMetaMemberPtr & member, meta->ormMetaMembers())
                {
                    QByteArray array = member->entityName().toLatin1();
                    row->setProperty(array, query.value(member->fieldName()));
    ...
    public:
        ...
        static inline QSqlError fetchByQuery(T& t, QSqlDatabase* database, const QdOrmWhere& condition)
        {
            using OriginalType = typename std::remove_reference<typename std::remove_pointer<typename T::value_type>::type>::type;
            const QdOrmClass* meta = QdOrmFactory::Get()->getAppointClass(OriginalType::GetTableName());
            Q_ASSERT_X(database != nullptr && meta != nullptr, "QdOrmDaoFetchContainerPtr::fetchByQuery()"
                , "database or meta is nullptr");
            t.clear();
    ...
    public:
        static inline bool exist(T& t, QSqlDatabase* database)
        {
            typedef typename std::conditional<std::is_pointer<typename T::value_type>::value, QdOrmDaoFetchContainerPtr<T>, QdOrmDaoFetchContainerGeneric<T>> ::type type_1;
            typedef typename std::conditional<is_smart_ptr<typename T::value_type>::value, QdOrmDaoFetchContainerPtr<T>, type_1>::type type_2;
        ...
        {
            typedef typename std::conditional<std::is_pointer<typename T::value_type>::value, QdOrmDaoFetchContainerPtr<T>, QdOrmDaoFetchContainerGeneric<T>> ::type type_1;
            typedef typename std::conditional<is_smart_ptr<typename T::value_type>::value, QdOrmDaoFetchContainerSmartPtr<T>, type_1>::type type_2;
