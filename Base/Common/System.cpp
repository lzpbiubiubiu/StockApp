...
#ifdef _DEBUG
        QString program = "\"" + appPath + "PosServer.exe\"";
        ShellExecute(NULL, TEXT("open"), program.toStdWString().c_str(), NULL, NULL, SW_SHOW);
#else
        QString program = "\"" + rootPath + "Launcher.exe\"";
        LOG_INFO(QStringLiteral("start launcher: %1").arg(program).toStdString());
        ShellExecute(NULL, TEXT("open"), program.toStdWString().c_str(), NULL, NULL, SW_SHOW);
#endif
#endif
    }
